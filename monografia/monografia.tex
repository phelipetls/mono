...
\section{Referencial Metodológico}
    ...
    Se todos os o módulo dos autovalores da matriz $ A_1 $ são menores que 1, o modelo é dito estável e converge assintoticamente para
    ...
    Logo, a distribuição de probabilidades de $ y_t $ é unicamente determinada pela distribuição do termo de erro $ u_t $, que se assume ser gaussiano. Esse resultado pode ser generalizado para AR(p), dado que qualquer processo AR(p) pode ser representado como um processo AR(1) \cite[p.~14-18]{lutkepool}, e serve para demonstrar que um AR(p) estável é estacionário no sentido amplo \cite[p.~25-26]{lutkepool}.
