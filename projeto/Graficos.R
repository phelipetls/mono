...
# -----------------------------------------------
df <- fread("../dados/series_economicas.csv") %>% as_tibble %>%
...
#       ggsave("IHH.pdf", device = cairo_pdf, dpi = 300)

# plt + geom_line(aes(date, Inflacao)) +
#       ggsave("Inflacao.pdf", device = cairo_pdf, dpi = 300)

# plt + geom_line(aes(date, Selic)) +
#       ggsave("Selic.pdf", device = cairo_pdf, dpi = 300)

# plt + geom_line(aes(date, Inadimplencia)) +
#       ggsave("Inadimplencia.pdf", device = cairo_pdf, dpi = 300)

# plt + geom_line(aes(date, Compulsorio)) +
#       ggsave("Compulsorio.pdf", device = cairo_pdf, dpi = 300)

# plt + geom_line(aes(date, ProducaoIndustrial_IPEADATA)) +
#       ggsave("ProducaoIndustrial_IPEADATA.pdf", device = cairo_pdf, dpi = 300)
